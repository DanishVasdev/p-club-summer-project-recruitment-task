...
// You can edit this contract as much as you want. A template is provided here and you can change the function names and implement anything else you want, but the basic tasks mentioned here should be accomplished.
    ...
    function getCompoundInterest(uint256 principal, uint rate, uint time) public pure returns(uint256) {
        ...
        while (time > 0) {
             if (time % 2 == 1) {
                  principal = 100*principal+principal * rate;
                  principal/=100;
                  time -= 1;
            } else {
    ...
    function veiwDues(address creditor) public isOwner() returns(uint256){
          return loans[creditor];
    }

    function settleDues(address creditor) public isOwner() returns(uint256){
        if(getOwnerBalance()>loans[creditor]){
        sendCoin(creditor,loans[creditor],owner);
        loans[creditor]=0;
        return loans[creditor] ;
        }
        else{
